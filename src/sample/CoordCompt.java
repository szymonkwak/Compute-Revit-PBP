...
public class CoordCompt {
    ...
    public static double computeCoordN(PointNE point, double distance, double azimuthDegree){
        //double N_PBP = glob1.getN() + ( CoordCompt.DistanceBtwPoints(loc1,0,0) * Math.cos(p1_PBP_AzimRad) );
        return point.getN() + ( distance * Math.cos(degreesToRad(azimuthDegree)) );
    }
    public static double computeCoordE(PointNE point, double distance, double azimuthDegree) {
        return point.getE() + (distance * Math.sin(degreesToRad(azimuthDegree)));
    }

    private static double degreesToRad(double a){
